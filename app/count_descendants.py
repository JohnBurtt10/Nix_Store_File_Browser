...
def count_descendants(hydra, raw_data, reverse_dependency_weight_dict, file_size_reverse_dependency_weight_dict, references_dict, file_size_dict):
    """
    ...
    for reference in references:
        ...
        (reverse_dependency_weight_dict[key], file_size_reverse_dependency_weight_dict[key]) = _count_descendants(
            hydra, reference, references_dict, file_size_dict)

    return reverse_dependency_weight_dict, file_size_reverse_dependency_weight_dict


def _count_descendants(hydra, package, references_dict, file_size_dict):
    if package is None:
        return (0, 0)
    ...
    for child in references:
        try:
            child_file_size = file_size_dict[package]
        except:
        ...
        if child != package:
            (child_descent_count, child_descent_file_size) = _count_descendants(
                hydra, child, references_dict, file_size_dict)
