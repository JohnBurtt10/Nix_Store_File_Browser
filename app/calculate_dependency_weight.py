    ...
    for dependency in references:
        if dependency in count_ancestor_cache:
            ...
            weight, weight_key, nodes, target_count, total_file_size, count_file_size, target_file_size = asyncio.run(
                _count_ancestors(hydra, dependency, 0, 0, references_dict, file_size_dict))
...
def calculate_dependency_weight(hydra, project_name, jobset, references_dict, file_size_dict):
    ...
    for eval_info in evals_info:
        ...
        with tqdm(total=len(filtered_builds), desc="Computing dependency weight", unit="builds") as pbar:
